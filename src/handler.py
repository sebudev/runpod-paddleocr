...
def _looks_like_base64(s):
    if not s or len(s) < 50:
        return False
    try:
        base64.b64decode(s[:100] + "=" * (4 - len(s[:100]) % 4) if len(s[:100]) % 4 else s[:100])
        return True
    except Exception:
        return False
...
def initialize_pipeline():
    ...
    try:
        ...
        logger.info(
            "Initializing PaddleOCRVL (version=%s, cache_dir=%s)",
...
def handler(job):
    ...
    tasks = job_input.get("tasks", DEFAULT_TASKS)
    output_format = job_input.get("output_format", OUTPUT_FORMAT)
    max_new_tokens = int(job_input.get("max_new_tokens", MAX_NEW_TOKENS))
    ...
    try:
        for idx, (source_type, source_val) in enumerate(sources):
            ...
            logger.info("Predicting on %s (task=%s)", local_path, tasks)
